...
# Config.add_plugin_config(
#     "black_word", "IGNORE_GROUP", [], help_="退出群聊惩罚中忽略的群聊, 即不可以退出的群聊", default_value=[]
# )
...
async def _():
    ...
    ** 惩罚机制 **
    ...
    惩罚前包含容忍机制, 在指定周期内可以容忍偶尔少次数的敏感词只可以进行警告提醒

    多次触发同级惩罚可以使惩罚等级提高, 即惩罚自动提级机制
    ...
    备注：
        ...
        该功能为测试阶段, 如果你有被误封情况, 请联系管理员, 可以从数据库中提取出你的数据进行审核后判断

        目前该功能暂不完善, 部分情况可以由管理员鉴定, 请注意对灵梦的发言
    ...
    A = BuildImage(
        max_width, max_height, font="NotoSerifSC-Bold.otf", font_size=24, color="#E3DBD1"
    )
    A.text((10, 10), text)
