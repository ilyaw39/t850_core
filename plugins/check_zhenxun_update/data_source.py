    ...
    if data:
        ...
        if _version != latest_version:
            ...
            if await AsyncHttpx.download_file(tar_gz_url, zhenxun_latest_tar_gz):
                ...
                logger.warning(f"下载灵梦最新版本失败...版本号：{latest_version}")
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
        ...
        await bot.send_private_msg(
            user_id=int(list(bot.config.superusers)[0]), message=f"自动获取灵梦版本失败...."
        )
    return 999, ''
...
def _file_handle(latest_version: str) -> str:
    ...
    logger.info("开始解压灵梦文件压缩包....")
    tf = tarfile.open(zhenxun_latest_tar_gz)
    tf.extractall(temp_dir)
    logger.info("解压灵梦文件压缩包完成....")
...
async def get_latest_version_data() -> dict:
    for _ in range(3):
        try:
            ...
            logger.error(f"检查更新灵梦获取版本失败 {type(e)}：{e}")
    return {}
