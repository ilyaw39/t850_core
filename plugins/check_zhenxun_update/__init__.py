...
update_zhenxun = on_command("检查更新灵梦", permission=SUPERUSER, priority=1, block=True)
...
async def _(bot: Bot):
    try:
        ...
        if error:
            logger.error(f"更新灵梦未知错误 {error}")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]), message=f"更新灵梦未知错误 {error}"
            )
    except Exception as e:
        logger.error(f"更新灵梦未知错误 {type(e)}：{e}")
        await bot.send_private_msg(
            user_id=int(list(bot.config.superusers)[0]),
            message=f"更新灵梦未知错误 {type(e)}：{e}",
        )
    else:
        if code == 200:
            await bot.send_private_msg(
