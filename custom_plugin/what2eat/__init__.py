...
add_group = on_command("添加菜品", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=3, block=True)
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not args:
        await add_group.finish("没输入你要添加的菜品呢~")
    ...
    logger.info(f"User {user_id} 添加了 {new_food} 到菜单")
...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    user_id = str(event.user_id)
    logger.info(f"Superuser {user_id} 添加了 {new_food} 到基础菜单")
